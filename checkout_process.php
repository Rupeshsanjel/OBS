...
if (isset($_SESSION["uid"])) {
    ...
    $sql0="SELECT bid_id from `bid_info`";
    $runquery=mysqli_query($con,$sql0);
    if (mysqli_num_rows($runquery) == 0) {
        echo( mysqli_error($con));
        $bid_id=1;
    }else if (mysqli_num_rows($runquery) > 0) {
        $sql2="SELECT MAX(bid_id) AS max_val from `bid_info`";
	...
	$sql = "INSERT INTO `bid_info` 
	(`bid_id`,`user_id`,`f_name`, `email`,`address`, 
    ...
    if(mysqli_query($con,$sql)){
        ...
        while($i<=$total_count){
            ...
            VALUES (NULL, '$order_id','$prod_id','$sub_total')";
